...
const Reviews = ({
  ...
  useEffect(() => {
    async function fetchData() {
      const resReservedUser = await axiosInstance.get(
        `/reservations/${localStorage.getItem("userId")}`
      );
      setUserReservations(resReservedUser.data.data);
    }
    fetchData();
  }, []);
      ...
      <div className={`container mt-16 mx-10    ${design.scroll}`}>
            ...
            {isArabic ? (
              <>
                <span className="text-custom mt-1 dark:text-[#CBB7A4]">
                  {room && room.ratingAvg ? room.ratingAvg : room.ratingAvg}
                </span>
                <span>
                      ...
                      {review.date && (
                        <p className="text-primary font-semibold dark:text-PrimaryDark">
                          {format(new Date(review.date), "MMMM yyyy", {
                            locale:
                              localStorage.getItem("lang") == "ar" ? ar : enUS,
                          })}
                        </p>
                      )}
                      {isShow &&
        ...
        {showConfirm && (
          <Confirm
            ...
            message={t("reviews.delete-review")}
          />
        )}
      </div>
    </>
